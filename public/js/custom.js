...
jQuery(document).ready(function($){
	...
	if($().validate) {
		...
		$('form[name="frmcontact"]').submit(function () {
			...
			var This = $(this);
			if($(This).valid()) {
			$('#ajax_contact_msg').append('<div id="modal" class="animate bounceInLeft"><h1>Espere un momento por favor ...</h1></div>');
				...
				$.ajax({
					 ...
					 error: function (xhr, status, error) {
					  ...
					  success: function (response) {
					  	$('.cerrarmodal').css("visibility","visible");
...
jQuery(document).ready(function(){
	jQuery(".cerrarmodal").click(function() {
     		jQuery('.cerrarmodal').css("visibility","hidden");
